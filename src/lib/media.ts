...
export function optimizedImageUrl(source?: string | null, width = 1600, quality = 72, height?: number) {
  if (!source || width <= 0 || !source.includes(SUPABASE_PUBLIC_OBJECT)) return source || '';
  try {
    ...
    url.searchParams.set('resize', height ? 'contain' : 'cover');
