...
type CaseItem = {
  ...
  thesis_media?: { file_url: string; alt_text: string | null; display_order: number }[];
};
...
export default function BeautifulSuccessCases() {
  ...
  useEffect(() => {
    ...
    supabase.from('theses')
      .select('id,title,subheadline,short_explanation,thesis_media(file_url,alt_text,display_order),thesis_impact_scales!inner(scale_slug)')
      .eq('status', 'published')
  ...
  return (
    <section className="award-home-section success-cases">
...
function CaseCard({ item }: { item: CaseItem }) {
  const media = [...(item.thesis_media || [])].sort((a, b) => a.display_order - b.display_order)[0];
