...
export default function AwardPage() {
  ...
  return (
    <main>
      ...
      <section className="ibs-section"><div className="award-section-title"><p className="award-label">{content.recognition[0][0]}</p><h2>{p.oneStandard}</h2></div><div className="recognition-list">{content.recognition.map(([title, copy], index) => <article key={title}><span>0{index + 1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></section>
    </main>
  );
}
