...
type Logo = { id:string; section_key:string; media_url:string; media_path:string|null; display_order:number };
...
export default function PartnerLogosAdmin() {
  ...
  async function load() {
    const { data, error: loadError } = await supabase.from('homepage_sections').select('id,section_key,media_url,media_path,display_order').like('section_key', 'partner_logo_%').order('display_order');
  ...
  async function upload(file: File) {
    ...
    const inserted = await supabase.from('homepage_sections').insert({ section_key: `partner_logo_${crypto.randomUUID()}`, media_url: mediaUrl, media_path: path, display_order: logos.length, is_active: true });
